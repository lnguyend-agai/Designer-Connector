...
// For 3Dfindit provider
// To hide the specific button
let injectable_css = document.createElement("style");
injectable_css.append("[id=\"j_id_2_2a:1:j_id_2_2b\"] { display: none !important; }");
injectable_css.setAttribute("type", "text/css");
document.head.appendChild(injectable_css);

// Create observer to observe the new mainContainer and remove specific logo
const observer = new MutationObserver(() => {
    var autodesk_button = document.querySelector('[title="Autodesk"]')
    if (autodesk_button !== null) {
        autodesk_button.remove();
    }
});
